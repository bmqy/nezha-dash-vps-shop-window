...
export default function ServerCard({ now, serverInfo }: { now: number; serverInfo: NezhaServer }) {
  ...
  return online ? (
    ...
    >
      ...
      <div className="flex items-center gap-2 -mt-2 lg:hidden">{parsedData?.billingDataMod && <VSWBillingInfo parsedData={parsedData} />}</div>
    ...
    >
      ...
      <div className="flex items-center gap-2 lg:hidden">{parsedData?.billingDataMod && <VSWBillingInfo parsedData={parsedData} />}</div>
      {parsedData?.planDataMod && <PlanInfo parsedData={parsedData} />}
    </Card>
  )
}
