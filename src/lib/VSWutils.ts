...
export function getDaysBetweenDatesWithAutoRenewal({ autoRenewal, cycle, startDate, endDate, amount }: BillingData): {
  days: number
  cycleLabel: string
  ...
  if (autoRenewal !== "1") {
    return {
      ...
      remainingPrice: getRemainingPrice(getDaysBetweenDates(endDate, new Date(nowTime).toISOString()), amount, cycleLabel = '年'),
    }
  }
  ...
  return {
    ...
    remainingPrice: getRemainingPrice(diff, amount, cycleLabel = '年'),
  }
}
...
export function getRemainingPrice(days: number, price: string, cycle: '年' | '半年' | '季' | '月'): string {
  const cycleValues: {
    "年": number,
    "半年": number,
    "季": number,
  ...
  let remainingPrice = '0.00';
  const priceValue = price.replace(/[$￥P€]/g, '');
  const priceUnit = price.match(/[$￥P€]/g) ? price.match(/[$￥P€]/g)?.[0] : '$';
  const rmp = parseFloat(priceValue) / cycleValues[cycle]  * days;
  if(!rmp){
    return remainingPrice;
  }
  return remainingPrice = `${priceUnit}${rmp.toFixed(2)}`;
}
